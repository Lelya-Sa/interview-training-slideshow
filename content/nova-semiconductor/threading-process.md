...
### 97) What is the difference between a **process** and a **thread**?
...
### 98) When do you choose **`Task`** over raw **`Thread`** in .NET?
...
### 99) What is the **thread pool** and why does .NET use it?
...
### 100) What does the **`lock`** statement do?
...
### 101) How do **`Monitor`** and **`lock`** relate?
...
### 102) When do you use **`SemaphoreSlim`**?
...
### 103) **`Mutex`** vs **`lock`** — when is Mutex needed?
...
### 104) What is **`ReaderWriterLockSlim`** for?
...
### 105) What does **`volatile`** guarantee in C#?
...
### 107) Name **thread-safe collections** you would use in production.
...
### 108) **`BlockingCollection<T>`** — typical use case?
...
### 109) **`Parallel.For`** — when is it appropriate?
...
### 110) **`Task.Run`** vs **`Task.Factory.StartNew`** — junior-safe answer?
...
### 111) How do **`CancellationToken`** and cooperative cancellation work?
...
### 112) **Async/await** vs **multithreading** — how do you explain the difference?
...
### 113) What causes **deadlock** and how do you prevent it?
...
### 116) How do you **start and monitor an external process** safely?
...
### 117) **`WaitHandle`** — what is it used for?
...
### 118) **Producer-consumer** pattern in C# — one clean implementation?
